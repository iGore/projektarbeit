...
Die funktionale Validierung gliedert sich in zwei Stufen. Auf Zielseite wird die Implementierung durch \texttt{go build}, \texttt{go vet} und \texttt{go test} abgesichert; der eigentliche, manuelle I/O-Vergleich gegen die Node.js-Quellimplementierung an konkreten Beispieleingaben ist als abschließender Evaluationsschritt geplant und steht zum Stand der Arbeit noch aus. Tabelle~\ref{tab:io-vergleich} fasst den derzeitigen Build- und Teststatus für alle acht identifizierten Funktionalitäten des Lizenz-Checkers zusammen.

\begin{table}[H]
\centering
\begin{tabular}{|p{0.5cm}|p{4.2cm}|p{4.0cm}|p{2.5cm}|}
\hline
\textbf{F} & \textbf{Funktionalität} & \textbf{Kernverhalten (Spec-Grundlage)} & \textbf{Build/Test} \\
\hline
001 & CLI-Optionsmodell & Flags parsen, normalisieren, \texttt{-{}-help}/\texttt{-{}-version} behandeln & bestanden \\
\hline
002 & Abhängigkeitsgraph laden & \texttt{npm ls}-Output traversieren, Dev-/Depth-Filter anwenden & bestanden \\
\hline
003 & Graph abflachen & Rekursives Inventar, Deduplizierung, Zirkel-Guard & bestanden \\
\hline
004 & Lizenzerkennung & Precedence Metadata $\to$ README $\to$ Datei; \texttt{*}-Marker; \texttt{Custom:} & bestanden \\
\hline
005 & Filter und Policy & SPDX-Ausschlüsse, \texttt{failOn}/\texttt{onlyAllow}-Exits, Private-Filter & bestanden \\
\hline
006 & Ausgabe-Rendering & JSON, CSV, Markdown, Tree, Summary; Dateiexport & bestanden \\
\hline
007 & Custom-Format & \texttt{customPath} laden, JSON-Konfiguration, Feld-Defaults & bestanden \\
\hline
008 & Stack-Utility & Lifecycle-Operationen des Hilfsdatentyps & bestanden \\
\hline
\end{tabular}
\caption{Build- und Teststatus für alle acht identifizierten Funktionalitäten des Lizenz-Checkers nach Abschluss von Phase~3 (\texttt{go build ./...}, \texttt{go vet ./...}, \texttt{go test ./...}). Der abschließende manuelle I/O-Vergleich gegen die Node.js-Quellimplementierung steht noch aus.}
\label{tab:io-vergleich}
\end{table}

Alle acht Funktionalitäten wurden im Evaluationsdurchlauf mit \texttt{go test ./...} geprüft; sämtliche Unit- und Integrationstests bestehen, \texttt{go vet ./...} meldet keine Befunde und \texttt{go build ./...} erzeugt ein fehlerfreies Binary. F-006 (Ausgabe-Rendering) ist dabei ausschließlich durch Integrationstests unter \texttt{test/integration/render\_export/} abgedeckt, da das Renderpaket selbst keine eigenen Testdateien enthält. Damit ist die Zielimplementierung gegen die in den Spezifikationen festgehaltenen Akzeptanzszenarien abgesichert; eine empirische Aussage über die Verhaltensgleichheit zwischen Quell- und Zielsystem bleibt jedoch dem ausstehenden manuellen I/O-Vergleich vorbehalten. Dieser zweistufige Ansatz ist für den PoC-Rahmen bewusst pragmatisch: Er ist ohne zusätzliche Testinfrastruktur durchführbar und fokussiert auf die wesentliche Frage, ob die migrierte Implementierung das fachliche Verhalten des Originals erhält \parencite{nunes2025maintainability,rodriguezcardenas2026behelm}.
